...
python ${base_dir}/tools/mark_word_boundary.py < ${koen_fbase}.tok.clean.en > ${koen_fbase}.tok.clean.wb.en
python ${base_dir}/tools/mark_word_boundary.py < ${koen_fbase}.tok.clean.ko > ${koen_fbase}.tok.clean.wb.ko

# Learn Shared BPE
echo "Learning BPE with 32000. This may take a while..."
cat "${koen_fbase}.tok.clean.wb.ko" "${koen_fbase}.tok.clean.wb.en" | \
${tools_dir}/subword-nmt/learn_bpe.py -s 32000 -t > "bpe32k.wb.shared"

echo "Apply BPE with 32000 to tokenized files..."
for lang in ko en; do
  outfile=${koen_fbase}.tok.clean.wb.bpe32k.${lang}
  ${tools_dir}/subword-nmt/apply_bpe.py -c "bpe32k.wb.shared" < ${koen_fbase}.tok.clean.wb.${lang} > "${outfile}"
done

# Create vocabulary file for BPE
cat "${koen_fbase}.tok.clean.wb.bpe32k.ko" "${koen_fbase}.tok.clean.wb.bpe32k.en" | \
${tools_dir}/subword-nmt/get_vocab.py | cut -f1 -d ' ' > vocab.wb.bpe32k

cp vocab.wb.bpe32k vocab.wb.bpe32k.ko
cp vocab.wb.bpe32k vocab.wb.bpe32k.en
