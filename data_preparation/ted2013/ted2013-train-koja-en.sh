...
cat ${koen_fbase}.tok.ko ${jaen_fbase}.tok.ja > "${merge_fbase}.tok.koja"
cat ${koen_fbase}.tok.en ${jaen_fbase}.tok.en > "${merge_fbase}.tok.en"
...
${tools_dir}/mosesdecoder/scripts/training/clean-corpus-n.perl ${merge_fbase}.tok koja en "${merge_fbase}.tok.clean" 1 $MAX_SEN_LEN

# Learn Shared BPE
echo "Learning BPE with 32000. This may take a while..."
cat "${merge_fbase}.tok.clean.koja" "${merge_fbase}.tok.clean.en" | \
${tools_dir}/subword-nmt/learn_bpe.py -s 32000 -t > "bpe32k.shared"

echo "Apply BPE with 32000 to tokenized files..."
for lang in koja en; do
...
cat "${merge_fbase}.tok.clean.bpe32k.koja" "${merge_fbase}.tok.clean.bpe32k.en" | \
