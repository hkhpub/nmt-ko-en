...
SPM_TRAIN=/usr/local/bin/spm_train
SPM_ENCODE=/usr/local/bin/spm_encode
...
# Train and encode
$SPM_TRAIN --input=${CORPUS_DIR}/train/train.eu,${CORPUS_DIR}/train/train.en -model_prefix=sp${VOCAB_SIZE} --vocab_size=`expr $VOCAB_SIZE + 3`
...
# Make vocab file
cp train/train.eu train/train.tmp
cat train/train.en >> train/train.tmp
python $SCRIPT_DIR/make_vocab.py < train/train.tmp > vocab/en_eu.all
head -${VOCAB_SIZE} vocab/en_eu.all > vocab/en_eu.${VOCAB_SIZE}
rm train/train.tmp
