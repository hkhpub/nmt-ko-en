...
class SubwordTextEncoder(TextEncoder):
  ...
  def decode(self, ids, strip_extraneous=False):
    ...
    return unicode_to_native(
        tokenizer.decode(self.subtoken_ids_to_tokens(ids)))
  ...
  def subtoken_ids_to_subtoken_string(self, subtoken_ids):
    concatenated = " ".join(
      [self._subtoken_id_to_subtoken_string(s) for s in subtoken_ids])
    return concatenated
