...
def decode_and_encoder_outputs(name,
                               model,
                               sess,
                               trans_file,
                               ref_file,
  ...
  if decode:
    ...
    start_time = time.time()
    num_sentences = 0
    trans_f = codecs.open(trans_file, 'w+', 'utf-8')
    ...
    while True:
      try:
        nmt_outputs, _, encoder_outputs = model.decode(sess)
        if beam_width == 0:
          nmt_outputs = np.expand_dims(nmt_outputs, 0)

        batch_size = nmt_outputs.shape[1]
        num_sentences += batch_size
        sent_vec_batch = tf.math.reduce_mean(encoder_outputs, axis=0)
        np.savetxt(trans_f, sent_vec_batch.eval(), fmt='%.10e')
